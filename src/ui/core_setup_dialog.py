"""内核安装向导：首次在受支持平台上运行时，要求阅读并同意 SnowLuma 的
EULA / LICENSE / 隐私说明，然后从官方 release 下载安装内核。"""
...
class _SetupThread(QThread):
    ...
    def run(self):
        try:
            ...
            logger.debug("内核向导任务失败 mode=%s", self._mode, exc_info=True)
...
class CoreSetupDialog(QDialog):
    def __init__(self, settings, parent=None):
        ...
        layout.addWidget(SubtitleLabel("使用前请阅读并同意以下条款"))
        intro = QLabel(
            "本程序的消息监听依赖 SnowLuma 官方内核（专有组件，不随本程序分发）。"
        ...
        self._install_btn = PrimaryPushButton("同意并安装内核")
